...
void printpagetable(pde_t *pgdir){
  int i;
  int j;
  int size = 1024;
  for(i =0; i<size; i++){
    ...
    if((pgdir[i]& PTE_P) != 0){
      ...
      pde_t *pde;
      pde = &pgdir[PDX(i)];
      pgtab = (pte_t*)PTE_ADDR(pgdir[i]);
      cprintf(".. %d: pde 0x%p pa 0xxit%p\n", i, pgdir[i], PTE_ADDR(pgtab));
      if(((*pde & PTE_P) &&(*pde & PTE_U) && (*pde &PTE_W))!= 0)
      {
        for(j =0; j<size; j++){
          if((pgtab[j] & PTE_P) &&(pgtab[j] & PTE_U) && ( pgtab[j] &PTE_W)!= 0)
            cprintf(".. .. %d: pde 0x%p pa ox%p\n", j, pgdir[j], PTE_ADDR(pgtab));
        }
      }
  }
}
