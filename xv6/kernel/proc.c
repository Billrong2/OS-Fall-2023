...
/// End of Mechanism, Algorithm from
...
found:
  p->state = EMBRYO;  // if its unused
  p->pid = nextpid++; // pid will increase
  p->tickets = 10;    // new terminal will have a ticket.
  release(&ptable.lock);
...
int get_ticket_total()
{
  ...
  struct proc *p;
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  { // loop over every processes, we need this a lot
    if (p->state == RUNNABLE)
    {
      total_tickets++;
    }
...
// int rand(int *state) // implement rand()
// {
//   return *state = (int) *state * 48271 %0x7fffffff;
// }
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {                           // loop over every processes, we need this a lot
      if (p->state == RUNNABLE) // find total process number;
      {
  ...
  // //get_ticket_total();
  // total_num = get_ticket_total();
  // cprintf("total number is %d", total_num);
  // //swtch(&cpu->scheduler, proc->context);
  // switchkvm();
